...
function generateVolumetricData(
	...
	const folderName = `Mag_Field_${neighboursCount == 0 ? "00" : neighboursCount}_[${alphaValue}]_${loopCount}_S`;
	...
	const triggerCuda = (cudaFolderPath, cudaExecutionPath, byteFilePath, byteFileName, screenshotFolderPath) => {
		...
		if (isWindows) {
			...
			try {
				...
				if (!matched) {
					console.error("Could not find volumeFilename in the source code with any pattern!");
				...
				if (sourceCode === newSourceCode) {
					console.error("Warning: Source code was not modified by the replacement!");
					return;
				}
			...
			exec(cleanCommand, { cwd: cudaFolderPath, shell: true }, (err, stdout, stderr) => {
				if (err) {
				...
				exec(`"${cudaExecutionPath}"`, { cwd: workingDirectory, shell: true }, (err, stdout, stderr) => {
					if (err) {
	...
	const screenshotFolderPath = isWindows
		? `D:/work/professor/dipoleWebBack/${folderName}/${subFolderName}`
		: `/home/dahalp/prastut/volumeCreationServer/${folderName}/${subFolderName}`;
	triggerCuda(cudaPath, cudaExecutionPath, dataPath, fileName, screenshotFolderPath);
