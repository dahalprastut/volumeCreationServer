...
app.get(
	...
	(req, res) => {
		...
		const folderName = `Mag_Field_${req.params.neighboursNumber == 0 ? "00" : req.params.neighboursNumber}_[${
			req.params.alphaValue
		}]_${req.params.loopsNumber}${SDOType}`;
		...
		const filePath = `${__dirname}/${type == "SDO" ? SDOPath : ""}${folderName}/${subFolderName}/${filename}`; // Path to the file
...
function generateVolumetricData(
	...
	console.log("here?:,", `./${type === "SDO" && SDOPath}${folderName}/field3.json`);
	...
	const highestLoopLowerIntensityValue = UiLowestIntensityValueInHighestLoop || 100;
	const middleLoopLowerIntensityValue = UiLowestIntensityValueInMiddleLoop || 80;
	const lowestLoopLowerIntensityValue = UiLowestIntensityValueInLowestLoop || 80;
	...
	const middleIntensityStandardDeviation = UiMiddleIntensityStandardDeviation || 1;
	const lowestIntensityStandardDeviation = UiLowestIntensityStandardDeviation || 1;
	...
	const highestCount = Math.floor((highestPercentage / 100) * filteredNums.length);
	const middleCount = Math.floor((middlePercentage / 100) * filteredNums.length);
	...
	const findingGaussianArray = (max, dynamicCurveWidth, SD) => {
		...
		const scalingFactor = (dynamicCurveWidth - 1) / (maxThickness - minThickness);
	...
	const getTheGaussianIntenstity = (width, highestIntensity, lowestIntensity, SD) => {
		scaledIntensityArr = [];
		const start = -2;
		const end = 2;
		const mean = 0;
	...
	if (diffToHighest < diffToMidd && diffToHighest < diffToLowest) {
		findingGaussianArray(num_field[j], curveWidth, highestLoopStandardDeviation);
	} else if (diffToMidd < diffToHighest && diffToMidd < diffToLowest) {
		findingGaussianArray(num_field[j], curveWidthMiddle, middleLoopStandardDeviation);
	} else {
		findingGaussianArray(num_field[j], curveWidthLowest, lowestLoopStandardDeviation);
	}
	...
	const setVoxelAndNeighbors = (x, y, z, count, highestIntensity, lowestIntensity, SD) => {
	...
	const flattenAndWriteToFile = (data, filename, append = false) => {
		...
		const folderPath = path.posix.join(type == "SDO" ? SDOPath : "", folderName, subFolderName);
		...
		fs.writeFileSync(path.join(folderPath, `${filename}.byte`), Buffer.from(uint8Array), { flag }); // 'a' flag appends to the file
		...
		if (append) {
			...
			fs.writeFileSync(path.join(folderPath, `${filename}.txt`), descriptionText);
		}
	};
	...
	const triggerCuda = (
		...
		if (isWindows) {
			// Step 1: Copy files
			const cudaByteFilePath = path.join(cudaFolderPath, "data", `${byteFileName}.byte`);
			...
			try {
				if (!fs.existsSync(cudaSourcePath)) {
					throw new Error(`Source file not found at ${cudaSourcePath}`);
				}

				let sourceCode = fs.readFileSync(cudaSourcePath, "utf8");
				const newFilename = `data/${byteFileName}.byte`; // Include "data/" prefix since it's in data folder
				const pattern = /const char\s*\*\s*volumeFilename\s*=\s*"([^"]*)";/;

				if (!pattern.test(sourceCode)) {
					throw new Error("Could not find volumeFilename declaration");
				}

				const newSourceCode = sourceCode.replace(pattern, `const char *volumeFilename = "${newFilename}";`);

				fs.writeFileSync(cudaSourcePath, newSourceCode, "utf8");
				console.log(`Updated volumeFilename to "${newFilename}"`);

				// Update file timestamp to force rebuild
				fs.utimesSync(cudaSourcePath, new Date(), new Date());
			} catch (error) {
			...
			// Use REBUILD instead of Clean + Build to ensure full compilation
			const buildCommand = `${vcvarsPath} && MSBuild "${projectPath}" /t:Rebuild /p:Configuration=Debug /p:Platform=x64 /v:detailed /fl /flp:logfile=build.log;verbosity=detailed`;
			...
			const buildProcess = spawn(buildCommand, {
				cwd: cudaFolderPath,
				shell: true,
				stdio: "inherit", // This will show build output in real-time
			});
			...
			buildProcess.on("close", (code) => {
				...
				const runProcess = spawn(`"${cudaExecutionPath}" ${screenshotPathArg}`, {
					cwd: cudaFolderPath,
					shell: true,
					stdio: "inherit",
				});
			...
			fs.writeFileSync(cudaSourcePath, newSourceCode, "utf8");
			console.log(`Updated volumeFilename in CUDA source to ${byteFileName}.byte.`);
			...
			exec("make", { cwd: cudaFolderPath }, (err, stdout, stderr) => {
				...
				const screenshotPathArg = `--screenshot_path=${screenshotFolderPath}`;
				exec(`./volumeRender ${screenshotPathArg}`, { cwd: cudaFolderPath }, (err, stdout, stderr) => {
					if (err) {
	...
	const cudaExecutionPath = "D:/work/professor/cuda-samples/bin/win64/Debug/volumeRender.exe"; // Only for windows
	const dataPath = `./${type == "SDO" ? SDOPath : ""}${folderName}/${subFolderName}/${fileName}.byte`;

	// Define paths for trace and mag images
	const dataTracePath = `./${type == "SDO" ? SDOPath : ""}${folderName}/trace_image.png`;
	const dataMagPath = `./${type == "SDO" ? SDOPath : ""}${folderName}/mag_image.png`;
	const screenshotFolderPath = isWindows
		? `D:/work/professor/dipoleWebBack/${type == "SDO" ? SDOPath : ""}${folderName}/${subFolderName}`
		: `/home/dahalp/prastut/volumeCreationServer/${type == "SDO" ? SDOPath : ""}${folderName}/${subFolderName}`;
	triggerCuda(cudaPath, cudaExecutionPath, dataPath, dataTracePath, dataMagPath, fileName, screenshotFolderPath);
